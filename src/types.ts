/* SITE */
export type Url = `http://${string}` | `https://${string}` | `mailto:${string}`
type Path = `/${string}`
...
/* FEATURES */
export type BgType = 'plum' | 'dot' | 'rose' | 'particle' | 'ocean' | 'storm'
...
interface NowPlayingConfig {
  ...
  /**
   * Last.fm API key. Public per-key rate limit; safe to ship in client-side config.
...
export interface Features {
  ...
  /**
   * Whether to enable the now-playing widget, which shows the user's most recent
   * (or currently playing) Last.fm track.
   *
   * Set to `false` to hide the widget entirely. To enable, provide a `lastfmUsername`
